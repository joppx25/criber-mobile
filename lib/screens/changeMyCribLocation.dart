...
class ChangeMyCribLocation extends StatefulWidget {
  Function callback;
  ChangeMyCribLocation(this.callback);

  @override
  _changeMyCribLocationState createState() => new _changeMyCribLocationState();
}

class _changeMyCribLocationState extends State<ChangeMyCribLocation> {
  ...
  validateOnSubmit(context) async {
    if (validateAndSave()) {
      ...
      final currentUser = FirebaseDatabase.instance.reference().child("user").child(firebaseUser.uid);
      String data = json.encode({"email": firebaseUser.email, "name": currentUser.child('name').toString(), "subject": location, "description": description, "status": 2, "priority": 2});
  ...
  Future<bool> _onWillPop() {
    this.widget.callback(ChangeMyCribInfo(this.widget.callback))
        ?? false;
  }
    ...
    return new WillPopScope(
      ...
      child: Form(
        ...
        child: new Scaffold (
          ...
          key: _scaffoldKey,
          appBar: new ReusableWidgets().getAppBar(context, Strings.changeMyCribLocation, () => this.widget.callback(ChangeMyCribInfo(this.widget.callback))),
          body: new Center(
            child: new Padding (
